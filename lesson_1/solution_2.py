...
def IDS(problem, DLS_Function):
    ...
    for i in zero_to_infinity():
        ...
        if(result!='cut_off'):
            return result,total_time_cost,total_space_cost
...
def Recursive_DLS_GraphSearch(node, problem, limit, explored):
    ...
    for move in range(problem.action_space.n):
        ...
        time_cost +=1
        result,under_time,under_space = Recursive_DLS_GraphSearch(child,problem,limit-1,None)
    ...
    return 'cut_off' if cut_off_occurred else 'failure', time_cost, space_cost
